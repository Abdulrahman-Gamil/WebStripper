...
class EdgeWKeyPasswordExtractor:
    def __init__(self, profile_path: str, secret_key: bytes):
        """Initialize Edge password extractor with a custom profile path and decrypted key."""
    ...
    def extract_passwords(self, output_path=None) -> list:
        ...
        for login_db_path, profile in login_data_paths:
            print(
                f"{Fore.CYAN}Extracting from Edge profile: {profile}{Style.RESET_ALL}"
            )
